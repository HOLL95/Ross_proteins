...
"""values=evaluator.filter_knees("/home/henryll/Documents/Frontier_results/M4D2_inference_5/",filter_level="filter_pairwise_dominated", key="knee")
for key in values.keys():
    print(key)
    for key2 in values[key]:
        if key2 not in ["parameters", "scores"]:
            print(key2, [sci._utils.format_values(x, 2) for x in values[key][key2]])
values=evaluator.filter_knees("/home/henryll/Documents/Frontier_results/M4D2_inference_5/",filter_level="filter_other_score_dominated", key="knee")
for key in values.keys():
    print(key)
    for key2 in values[key]:
        if key2 not in ["parameters", "scores"]:
            print(key2, [sci._utils.format_values(x, 2) for x in values[key][key2]])"""
"""knees=evaluator.open_knees("/home/henryll/Documents/Frontier_results/M4D2_inference_5/", keyarg="knee_region_5")
values=evaluator.filter_knees(knees=knees, key="knee_plus_6", filter_level="ranking")          
for key in values.keys():
    print(key)
    for key2 in values[key]:
        if key2 not in ["parameters", "scores"]:
          print(key2,values[key][key2])"""
print(evaluator.grouping_keys)
desired_keys=[x for x in evaluator.grouping_keys if "FTACV" not in x or "ts" not in x]
print(desired_keys)
combos=list(itertools.combinations(desired_keys,2))
for i in range(2, len(combos)):
    savename="&".join(combos[i])
    evaluator.pareto_explorer("/home/henryll/Documents/Frontier_results/M4D2_inference_5/",combos[i][0], combos[i][1], savename=savename)
"""for key in values.keys():
    print(key)
    for key2 in values[key]:
        if key2 not in ["parameters", "scores"]:
...
evaluator.plot_all_fronts("/home/henryll/Documents/Frontier_results/M4D2_inference_5/set_18/fronts", plot_knee=True, colours=[sci._utils.colours[0]])
plt.show()
#all_knees=evaluator.get_all_knees("/home/henryll/Documents/Frontier_results/M4D2_inference_4/set_3/fronts")
#evaluator.pareto_explorer("/home/henryll/Documents/Frontier_results/M4D2_inference_4/set_3/fronts", evaluator.grouping_keys[3], evaluator.grouping_keys[4])
