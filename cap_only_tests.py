...
experiments_dict['FTACV']['3_Hz']['80']= {'E_start': np.float64(-0.8902481705989678), 'E_reverse': np.float64(-0.0005663458462876747), 'omega': np.float64(3.0347196895539086), 'phase': 0, 'delta_E': np.float64(0.07653233662675293), 'v': np.float64(0.05954572063421371)}
...
amps=["80","280"]
area_factor=1
best_fits1a=dict(zip(frequencies,[
...
best_fits=dict(zip(frequencies,[
    [-0.4245986175, 0.0413861721,     1.5523778321e+03, 3.5063811215e-10,         2.5703063109e+03, 1.2613741365e-04, -3.6247299571e-03, -1.1404172534e-03, -1.3143727704e-05, 3.0346811579,  0.5605496061],
    [ -0.4209999102, 0.0649566043,     828.5335246381,   2.1501098646e-10,         777.8466804005,   1.8833603227e-04, -3.6493718528e-04, -5.9560375601e-04, -4.2018232542e-06, 9.1041971133,  0.5089475239],
    [-0.4223992757, 0.076092246,      115.6594230467,   1.5371346136e-10,         255.1074992031,   1.4020628051e-04, 3.1897209566e-03,  -4.2256809206e-04, -1.6105495786e-05, 15.1735513299, 0.4012169576],
...
for i in range(0,len(frequencies)):
    ...
    for j in range(1, len(amps)):
        ...
        slurm_class.Faradaic_only=True
        ...
        potential=data[:,2]
        start=time1.time()
        test_full=slurm_class.dim_i(slurm_class.Dimensionalsimulate(best_fits[frequencies[i]], time))
        test=np.subtract(current, test_full)
        print(time1.time()-start)
        plt.plot(test)
        plt.show()
        axes=sci.plot.plot_harmonics(
                            Data_data={"time":time, "current":current*1e6,  "harmonics":list(range(1, 10))},
                            Sim_data={"time":time, "current":test*1e6, "potential":potential, "harmonics":list(range(1, 10))},
        ...
        plt.show()
        fig.savefig("{0}_harmonics.png".format(frequencies[i]))
        """
