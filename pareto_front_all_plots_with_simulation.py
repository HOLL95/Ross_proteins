...
class Surface:
    def __init__(self, plot_dict, min_dict, simdict):
        ...
        self.nc=10
        self.p_cs=colors = cmap(np.linspace(0, 1, self.nc))
        ...
        self.axis=axis
        self.simline={}
        self.freqcolours=freqcolours
        
        fig.set_size_inches(15,8)
        key="65_75"
        j=0
        pareto_x=100*(1-sci._utils.normalise(params_dict[key][j]["x"], self.min_dict["65"]))
        pareto_y=100*(1-sci._utils.normalise(params_dict[key][j]["y"], self.min_dict["75"]))
        self.scatter1=self.axis[0,1].scatter(pareto_x, pareto_y, color=self.freqcolours["75"], edgecolors="black")
        self.scatter2=self.axis[0,2].scatter(pareto_y, pareto_x, color=self.freqcolours["65"], edgecolors="black")

     
        for m in range(0, len(strfreqs)):
            self.simline[strfreqs[m]]={}
            for q in range(0, len(directions)):
                    self.axis[1,m].plot(simclass.data_dict[strfreqs[m]][directions[q]]["potential"],simclass.data_dict[strfreqs[m]][directions[q]]["current"], color=colours[m])
                    self.simline[strfreqs[m]][directions[q]],=self.axis[1,m].plot(simclass.data_dict[strfreqs[m]][directions[q]]["potential"],self.simdict[key][strfreqs[m]][directions[q]], color=self.p_cs[0])
                    self.axis[1,m].set_xlabel("Potential (V)")
                    if m==0:
                        self.axis[1,m].set_ylabel("Current (A)")
        ...
        self.size_pareto_step=5
        
        
    def update(self,frame):
        
        num_pareto_steps=50/self.size_pareto_step
        index=frame
        

        i=int(index//num_pareto_steps)
        j=int((index%num_pareto_steps)*self.size_pareto_step)
        
        
        
        freq1=str(combinations[i][0])
        col_pos=strfreqs.index(freq1)
       
        freq2=str(combinations[i][1])
        col_pos2=strfreqs.index(freq2)
        key="{0}_{1}".format(freq1, freq2)
        savedict[key]={}
        ...
        pareto_x=100*(1-sci._utils.normalise(params_dict[key][j]["x"], self.min_dict[freq1]))
        pareto_y=100*(1-sci._utils.normalise(params_dict[key][j]["y"], self.min_dict[freq2]))
        
       
            
        self.scatter1.remove()
        self.scatter2.remove()
        self.scatter1=self.axis[0,col_pos].scatter(pareto_x, pareto_y, color=self.freqcolours[freq2], edgecolors="black")
        self.scatter2=self.axis[0,col_pos2].scatter(pareto_y, pareto_x, color=self.freqcolours[freq1], edgecolors="black")
        #simulations=simclass.evaluate(get_params)
        for m in range(0, len(strfreqs)):                
            for q in range(0, len(directions)):
                #self.simline[strfreqs[m]][directions[q]].set_ydata(self.simdict[key][strfreqs[m]][directions[q]])
                self.simline[strfreqs[m]][directions[q]].remove()
                self.simline[strfreqs[m]][directions[q]],=self.axis[1,m].plot(simclass.data_dict[strfreqs[m]][directions[q]]["potential"],self.simdict[key][strfreqs[m]][directions[q]], color=self.p_cs[index%self.nc])
        return self.axis
               


import matplotlib.animation as animation
surf=Surface(plot_dict, min_dict, simdict)
ani = animation.FuncAnimation(fig=surf.fig, func=surf.update, frames=int(len(combinations)*(50/surf.size_pareto_step)), interval=200)

ani.save(filename="frontier_tests/animations.gif", writer="pillow")
plt.show()