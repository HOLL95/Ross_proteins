...
for i in range(0, len(frequencies)):
    ...
    harmonics=list(range(2, 20))
    #fig, ax=plt.subplots(len(harmonics),1)
    
    
    
    init_vals=[ -0.4279614654,0.02,     204.7075623261,   5.9999383440e-11,         721.1722868916,   1.0053023051e-04, -7.9868011396e-03, 4.1358395131e-06,  9.9973984312e-06, dictionary["omega"], dictionary["phase"],  dictionary["phase"],    0.5685817265,]
    ...
    time=slurm_class.calculate_times(sampling_factor=200, dimensional=True)
    test=slurm_class.dim_i(slurm_class.Dimensionalsimulate(init_vals, time))
    potential=slurm_class.get_voltage(time, dimensional=True)
    
    
    harms=sci.plot.generate_harmonics(time, test, one_sided=False, hanning=False, filter_val=0.5, harmonics=harmonics)
    DC=slurm_class.calc_DC_voltage(time, dimensional=True)
    current=np.real(harms[0,:])
    for i in range(1, len(harms)):
        current+=np.real(harms[i,:])
        plt.plot(time,test)
