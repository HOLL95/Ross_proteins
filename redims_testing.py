...
areas=[0.036, 0.07]
factors=[1,0.07/0.036]
plot_dict=dict(plot_func=np.abs, hanning=True,  xlabel="Time (s)", ylabel="Current ($\\mu$A)", remove_xaxis=True)
for k in range(0, len(areas)):
    for i in range(0,1):
        ...
        for j in range(1, len(amps)):
            ...
            time=data[:,0]
            potential=data[:,2]
            plot_dict["{0}_data".format(areas[k])]={"time":time, "current":slurm_class.nondim_i(current)*factors[k],  "harmonics":list(range(2, 10))}
axes=sci.plot.plot_harmonics(**plot_dict,
