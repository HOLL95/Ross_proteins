                    ...
                    if group_dict[groupkey]["experiment"]=="FTACV":
                        ...
                        if group_dict[groupkey]["match"]==cond_2:
                            if group_dict[groupkey]["type"]=="ts":
                                ...
                                data=data/(omega*deltaE)
                                sim=sim/(omega*deltaE)
                                if mode=="residual":
                                    plot_dict[groupkey]["axis"].plot(xaxis, data-sim)
                                else:
                                    plot_dict[groupkey]["axis"].plot(xaxis, data)
                                    plot_dict[groupkey]["axis"].plot(xaxis, sim, linestyle="--", color="black", alpha=0.5)
...
if save==True:
    ...
    for m in range(0, 11):
        ...
        ax_client=np.load("/home/henryll/Documents/Frontier_results/M4D2_inference/set_{0}/ax_client.npy".format(m), allow_pickle=True).item()["saved_frontier"]
    ...
    np.save("init_pareto_results/save_points.npy", results)
else:
    results=np.load("init_pareto_results/save_points.npy", allow_pickle=True).item()
...
experiment_keys=dict(zip(grouping_keys, ["FTACV-3_Hz-80", "FTACV-3_Hz-280","FTACV-3_Hz-80","FTACV-3_Hz-280", "SWV-135_Hz-anodic","SWV-135_Hz-cathodic"]))
