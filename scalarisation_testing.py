...
freqs=[65, 75, 85, 100, 115, 125, 135, 145, 150, 175, 200, 300,  400, 500][:7]
...
def hypervolume_scalarization(y, lambd):
    """
    Calculates the hypervolume scalarization for a given objective vector.

    Args:
        y (np.ndarray): Objective vector (y1, y2, ..., yk).
        lambd (np.ndarray): Weight vector (lambda1, lambda2, ..., lambdak).

    Returns:
        float: Scalarized value.
    """
    k = len(y)
    return np.min(np.maximum(0, y / lambd))**k


def sample_weight_vector(k):
    """
    Samples a weight vector from the positive orthant of the unit sphere.

    Args:
        k (int): The number of objectives.

    Returns:
        np.ndarray: A weight vector.
    """
...
def calculate_ck(k):
    """
    Calculates the constant ck which depends on dimensionality.

     Args:
         k (int): The number of objectives
    Returns:
        float: The value of ck

    """
    return np.pi**(k/2) / (2**k * gamma(k/2 + 1))


def hypervolume_indicator(Y, z, s):
    """
     Approximates the hypervolume indicator.

     Args:
        Y (list of np.ndarrays): A list of objective vectors.
        z (np.ndarray): The reference point
        s (int): number of samples

    Returns:
       float: An estimation of the hypervolume indicator.
    """
    k = len(z)
    ck = calculate_ck(k)
    scalarized_values = []
    for _ in range(s):
        lambd = sample_weight_vector(k)
        max_s = -np.inf
        for y in Y:
            scalarized = hypervolume_scalarization(y - z, lambd)
            max_s = max(max_s, scalarized)
        scalarized_values.append(max_s)

    return ck * np.mean(scalarized_values)
